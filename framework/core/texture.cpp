...
namespace framework
{
    ...
    Texture::~Texture()
    {
        ...
        {
            vkDestroyImage(lDevice->getDevice(), textureImage, nullptr);
        }

        if (textureImageMemory != VK_NULL_HANDLE)
        {
            vkFreeMemory(lDevice->getDevice(), textureImageMemory, nullptr);
        }

        if (textureImageView != VK_NULL_HANDLE)
        {
            vkDestroyImageView(lDevice->getDevice(), textureImageView, nullptr);
        }

        if (textureSampler != VK_NULL_HANDLE)
        {
            vkDestroySampler(lDevice->getDevice(), textureSampler, nullptr);
        }
    }
    ...
    void Texture::createSampler(VkSampler &sampler)
    {
        ...
        if (vkCreateSampler(lDevice->getDevice(), &samplerInfo, nullptr, &textureSampler) != VK_SUCCESS)
        {
            throw std::runtime_error("[Texture] Impossible to crete texture sampler");
        }
    }

    void Texture::createImageView(VkImage image, VkFormat format, VkImageAspectFlags aspectFlags, VkImageView &view)
    {
